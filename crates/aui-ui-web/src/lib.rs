...
impl WebAdapter {
    ...
    pub fn apply(
        ...
        let command: BridgeCommand =
            serde_json::from_str(payload).map_err(|err| format!("web decode failed: {err}"))?;
        ...
        serde_json::to_string(&snapshot).map_err(|err| format!("web encode failed: {err}"))
    }
}
...
mod tests {
    use super::*;
    use aui_agent_core::BridgeAttachment;
    use aui_agent_core::CoreRuntime;
    use aui_agent_core::{ModelCatalog, ProviderKind, ProviderRequest, Session};
    use aui_agent_core::{
